...
cmd2=f'mkdir -p ~/backup; cp -R /var/www/html/wordpress /home/{env.user}/backup/; sudo mysqldump -u root wordpress > /home/{env.user}/backup/{datestr}_dump.sql'
cmd3=f'sudo tar -cvf /home/{env.user}/{datestr}_backup.tar.gz ~/backup'
cmd4=f'lftp sftp://{env.user}:{env.pwd}@{env.host} -e "mkdir -f backup; put -O ~/backup /home/{env.user}/{datestr}_backup.tar.gz; quit"'
